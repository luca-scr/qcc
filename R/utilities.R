...
#' a \code{data.frame}) providing the observed data. If not provided, the input
#' to the following arguments must be objects defined in the calling
#' environment.
#' @param x a name from \code{data} or a vector of observed data values.
#' @param sample a name from \code{data} or a vector of sample indicators
...
#' group have fewer observations than others, \code{NA} values are used to fill
#' empty values.
#' @author Luca Scrucca
...
#' option in \code{numpy.histogram_bin_edges} available in Python.
#' 
#' 
...
#' poisson case.
#' 
#' This very simple test amounts to compute the test statistic \deqn{D = s^2 /
#' \sigma^2 \times (n - 1)} where \eqn{s^2} is the observed variance,
#' \eqn{\sigma^2} is the theoretical variance, and \eqn{n} is the number of
#' observations. The test statistic is the compared to the critical value of a
#' Chi-square distribution with \eqn{n-1} degrees of freedom.
#' 
...
#' either \code{"poisson"} or \code{"binomial"}. By default, if \code{size} is
...
#' Set or return options for the \pkg{qcc} package.
#' 
...
#' 
#' The available options are: \describe{
#' 
#' \item{list("exp.R.unscaled")}{a vector specifying, for each sample size, the
#' expected value of the relative range (i.e. \eqn{R/\sigma}) for a normal
#' distribution. This appears as \eqn{d_2} on most tables containing factors
#' for the construction of control charts.}
#' 
#' \item{list("se.R.unscaled")}{a vector specifying, for each sample size, the
#' standard error of the relative range (i.e. \eqn{R/\sigma}) for a normal
#' distribution. This appears as \eqn{d_3} on most tables containing factors
#' for the construction of control charts.}
#' 
#' \item{list("beyond.limits$pch")}{plotting character used to highlight points
#' beyond control limits.}
#' 
#' \item{list("beyond.limits$col")}{color used to highlight points beyond
#' control limits.}
#' 
#' \item{list("violating.runs$pch")}{plotting character used to highlight
#' points violating runs.}
#' 
#' \item{list("violating.runs$col")}{color used to highlight points violating
#' runs.}
#' 
#' \item{list("run.length")}{the maximum value of a run before to signal a
#' point as out of control.}
#' 
#' \item{list("bg.margin")}{background color used to draw the margin of the
#' charts.}
#' 
#' \item{list("bg.figure")}{background color used to draw the figure of the
#' charts.}
#' 
#' \item{list("cex")}{character expansion used to draw plot annotations
#' (labels, title, tickmarks, etc.).}
#' 
#' \item{list("font.stats")}{font used to draw text at the bottom of control
#' charts.}
#' 
#' \item{list("cex.stats")}{character expansion used to draw text at the bottom
#' of control charts.}
#' 
#' }
#' 
#' @param \dots the option to be set or retrieved. See details.
...
#' \code{.qcc.options} is modified and any modification will remain in effect
