                     ...
                     ...)
{
  ...
  if(!chart.all & (!is.null(newstats)))
    df <- df[seq(1, length(df$group)) > length(object$statistics),]
  ...
  if(any(object$rules == 1))
  {
    ...
    y1 <- if(length(lcl) == 1) rep(lcl, length(x1)) else
            c(lcl[seq(1, length(df$group))], length(df$group))
    y2 <- if(length(ucl) == 1) rep(ucl, length(x2)) else
            c(ucl[seq(1, length(df$group))], length(df$group))
    xp1 <- rep(x1, each=2)[-1]
    xp2 <- rep(x2, each=2)[-1]
  ...
  if(any(object$rules == 2))
  {
    ...
    {
      y1 <- c(limits.2sigma[rep(1, length(df$group)),1],
              limits.2sigma[length(df$group),1])
      y2 <- c(limits.2sigma[rep(1, length(df$group)),2],
