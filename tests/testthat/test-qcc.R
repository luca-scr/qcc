...
test_that("Creating xbar charts return qcc object with two control limits", {
  ...
  chart <- qcc(diameter[1:25, ], type = "xbar")
...
test_that("xbar.one charts produce the correct number of violations", {
  ...
  chart <- with(viscosity, qcc(viscosity[trial], type = "xbar.one"))
...
test_that("xbar end-to-end covers print, summary, plot, and limits-mode switch", {
  ...
  plotted <- plot.qcc(chart, add.stats = FALSE, fill = FALSE)
...
test_that("plot.qcc validates xtime class", {
  ...
  expect_error(
    plot.qcc(chart, xtime = rep("bad", total_groups)),
    "xtime must be"
  )
})
...
test_that("S-chart helpers keep expected structure and non-negative lower limits", {
  ...
  s_stats <- stats.S(grouped, subgroup_sizes)
  s_sd <- sd.S(grouped, subgroup_sizes)
  s_limits <- limits.S(
...
test_that("qcc validates required chart inputs and dimensions", {
  ...
  expect_error(qcc(c(1, 2, 3), type = "u"), "sample 'sizes' must be given")
  expect_error(qcc(grouped, type = "not_a_chart"), "invalid")
  expect_error(
    qcc(grouped, type = "xbar", std.dev = list(1)),
    "std.dev"
  )
  expect_error(
    qcc(grouped, type = "xbar", sizes = c(4, 4)),
    "sizes length doesn't match"
  )
  expect_error(
    qcc(grouped, type = "xbar", limits = "bad"),
...
test_that("p and np helpers remain internally consistent", {
  ...
  p_stats <- stats.p(counts, sample_sizes)
  np_stats <- stats.np(counts, sample_sizes)
  p_sd <- sd.p(counts, sample_sizes)
  np_sd <- sd.np(counts, sample_sizes)
  p_limits <- limits.p(p_stats$center, p_sd, sample_sizes, nsigmas = 3)
  np_limits <- limits.np(np_stats$center, np_sd, sample_sizes, nsigmas = 3)
...
test_that("R-chart helpers keep expected structure and enforce max subgroup size", {
  ...
  r_stats <- stats.R(grouped, subgroup_sizes)
  r_sd <- sd.R(grouped, subgroup_sizes)
  r_limits <- limits.R(
